...
export function TasksWorkspace() {
  ...
  const [isSavingLoginAccess, setIsSavingLoginAccess] = useState(false);
  ...
  const canManageLoginAccess = sessionUser?.role === 'admin' || sessionUser?.role === 'manager';
  ...
  const currentTourModule: 'tasks' | 'projects' | 'team' | 'reports' | 'analytics' | 'settings' =
    activeSection === 'blocked' || activeSection === 'today'
      ? 'tasks'
  ...
  function mapWorkspaceRoleToAuthRole(role: UserRole): AuthUser['role'] {
    if (role === 'admin') return 'admin';
    if (role === 'manager') return 'manager';
    return 'author';
  }

  async function saveProfileLoginAccess(user: User, password?: string) {
    const email = user.email?.trim();
    if (!email) {
      throw new Error('Email is required before enabling login access.');
    }

    if (!user.loginAccess?.authUserId && !password?.trim()) {
      throw new Error('A temporary password is required when login access is created.');
    }
    ...
    const response = await fetch('/api/auth/users', {
      ...
      body: JSON.stringify({
        id: user.loginAccess?.authUserId,
        name: user.name,
        email,
        role: user.loginAccess?.role ?? mapWorkspaceRoleToAuthRole(user.role),
        team: user.team,
        status: user.loginAccess?.status ?? 'active',
        password: password?.trim() || undefined
      })
    });
    const payload = (await response.json()) as { data?: AuthUser; error?: string };
    if (!response.ok || !payload.data) {
      throw new Error(payload.error || 'Failed to save login access.');
    }
  ...
  async function handleSaveSelectedProfileLoginAccess() {
    if (!selectedTeamSummary) return;
    if (!canManageLoginAccess) {
      setTeamProfileMessage('Sign in as a manager or admin to manage login access.');
      return;
    }

    setIsSavingLoginAccess(true);
    setTeamProfileMessage('');
    try {
      const saved = await saveProfileLoginAccess(selectedTeamSummary.user, profilePasswordDraft);
      setProfilePasswordDraft('');
      setTeamProfileMessage(`Login access saved for ${saved.email}.`);
    } catch (error) {
      setTeamProfileMessage(error instanceof Error ? error.message : 'Failed to save login access.');
    } finally {
      setIsSavingLoginAccess(false);
    }
  }
  ...
  const handleAddTeamMember = async () => {
    ...
    const email = teamMemberDraft.email.trim().toLowerCase();
    if (!name || !team) return;
    if (teamMemberDraft.loginAccessEnabled && (!email || !teamMemberDraft.temporaryPassword.trim())) {
      setTeamProfileMessage('Email and a temporary password are required to enable login access.');
      return;
    }
    if (teamMemberDraft.loginAccessEnabled && !canManageLoginAccess) {
      setTeamProfileMessage('Sign in as a manager or admin to create login access.');
      return;
    }
    ...
    const nextUser: User = {
      ...
      loginAccess: teamMemberDraft.loginAccessEnabled
        ? {
            enabled: true,
            role: teamMemberDraft.authRole,
            status: teamMemberDraft.authStatus
          }
        : undefined
    };
    ...
    if (teamMemberDraft.loginAccessEnabled) {
      try {
        await saveProfileLoginAccess(nextUser, teamMemberDraft.temporaryPassword);
        setTeamProfileMessage(`Team member added and login access enabled for ${email}.`);
      } catch (error) {
        setTeamProfileMessage(error instanceof Error ? error.message : 'Team member added, but login access was not created.');
      }
    } else {
      setTeamProfileMessage('Team member added without login access.');
    }
  ...
  return (
    <div className="shell">
      <header className="topbar" data-tour="shell.topbar">
        ...
        <div className="topbar-nav" data-tour="shell.module-nav">
          <div className="topbar-nav-group">
            ...
            <button className={activeSection === 'profiles' ? 'is-active' : ''} onClick={() => setActiveSection('profiles')}>Profiles</button>
      ...
      {isSetupWizardOpen && !showImportWizard ? (
                ...
                {setupWizardStep === 'work' ? (
                  <div className="setup-wizard-section">
                    ...
                    <div className="modal-grid">
                      ...
                      <label className="field"><span>Start Date</span><input type="date" value={draft.startAt} onChange={(event) => handleDraftChange('startAt', event.target.value)} /></label>
                      <label className="field"><span>End Date</span><input type="date" value={draft.endAt} onChange={(event) => handleDraftChange('endAt', event.target.value)} /></label>
                    </div>
      ...
      {showImportWizard ? <HrImportWizard onClose={closeRoleImport} onComplete={closeRoleImport} /> : null}
